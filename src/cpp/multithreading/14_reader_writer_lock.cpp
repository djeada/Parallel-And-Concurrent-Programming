...
public:
    void read(int thread_id) {
        ...
        std::this_thread::sleep_for(std::chrono::milliseconds(random_duration(500, 1000)));
    ...
    void write(int thread_id) {
        ...
        ++data;
        std::this_thread::sleep_for(std::chrono::milliseconds(random_duration(500, 1000)));
        std::cout << "Writer " << thread_id << " finished writing. New data: " << data << std::endl;
    }
...
void reader(SharedData& shared_data, int thread_id) {
    while (true) {
        ...
        std::this_thread::sleep_for(std::chrono::milliseconds(shared_data.random_duration(1000, 2000)));
    }
}
