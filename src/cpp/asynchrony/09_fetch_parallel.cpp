...
std::string fetch_async(const std::string& url) {
...
int main() {
    ...
    auto elapsed_time_sync = std::chrono::duration_cast<std::chrono::milliseconds>(std::chrono::steady_clock::now() - start_time_sync).count();
    std::cout << "\nSynchronous execution took " << elapsed_time_sync / 1000.0 << " seconds.\n";

    auto start_time_async = std::chrono::steady_clock::now();
    auto contents_async = fetch_all_async(URLS);
    auto elapsed_time_async = std::chrono::duration_cast<std::chrono::milliseconds>(std::chrono::steady_clock::now() - start_time_async).count();
    std::cout << "\nAsynchronous execution took " << elapsed_time_async / 1000.0 << " seconds.\n";
