...
public:
    ...
    void start() {
        ...
        while (true) {
            ...
            std::thread worker_thread(&Server::handle_connection, this, client_socket);
...
public:
    ...
    void start() {
        if (connect(worker_socket, (sockaddr *)&server_addr, sizeof(server_addr)) < 0) {
...
int main(int argc, char *argv[]) {
    if (argc < 3) {
        std::cerr << "Usage: " << argv[0] << " [server|worker|client] [port]" << std::endl;
        return 1;
    }
