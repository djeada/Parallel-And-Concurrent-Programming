...
public:
    ...
    Task get_return_object() { return {std::coroutine_handle<TaskPromise>::from_promise(*this)}; }
...
task resource_limited_operation(int id) {
    std::cout << "Worker " << id << " is entering the resource-limited operation\n";
    std::this_thread::sleep_for(std::chrono::milliseconds(rand() % 1500 + 500));
    std::cout << "Worker " << id << " is leaving the resource-limited operation\n";
    co_return;
}
