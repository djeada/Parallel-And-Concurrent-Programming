/*
This program demonstrates the use of fork() to create multiple processes and pipes to establish communication
between them in C++. A pipe allows for safe inter-process communication by providing a simple way to pass data
between processes. In this example, we create a producer process that generates data and writes it into the pipe,
and a consumer process that reads the data from the pipe.
...
The producer generates data and writes it into the pipe, while the consumer reads the data from the pipe and
