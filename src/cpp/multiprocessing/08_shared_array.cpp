...
shared memory in C++. We use the mmap system call to create a shared memory object
and allow the processes to access it directly.

The program creates two child processes that operate on the shared array. One child
...
processes in C++ using shared memory.
*/
...
int main() {
    int* shared_array = static_cast<int*>(mmap(NULL, sizeof(int) * ARRAY_SIZE, PROT_READ | PROT_WRITE, MAP_SHARED | MAP_ANONYMOUS, -1, 0));
