...
if (process.argv.length === 2) {
  ...
  process.on('message', (message) => {
    if (message === `grantLock${workerName === 'workerA' ? 'A' : 'B'}`) {
      console.log(`${workerName} acquired lock ${workerName === 'workerA' ? 'A' : 'B'}`);

      setTimeout(() => {
        console.log(`${workerName} trying to acquire lock ${workerName === 'workerA' ? 'B' : 'A'}`);
        process.send(`requestLock${workerName === 'workerA' ? 'B' : 'A'}`);
      }, 1000);
    } else if (message === `grantLock${workerName === 'workerA' ? 'B' : 'A'}`) {
      console.log(`${workerName} acquired lock ${workerName === 'workerA' ? 'B' : 'A'}`);
